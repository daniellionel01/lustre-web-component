...
var Runtime = class {
  constructor(root3, [model, effects], view2, update3) {
    ...
    this.#view = view2;
    this.#update = update3;
    this.#reconciler = new Reconciler(this.root, (event2, path, name2) => {
      const [events, msg] = handle(this.#events, path, name2, event2);
      this.#events = events;
      if (msg.isOk()) {
...
function do_remove_event(handlers, path, name2) {
  return remove(handlers, event(path, name2));
}
...
function handle(events, path, name2, event2) {
...
var make_component = ({ init: init2, update: update3, view: view2, config }, name2) => {
  ...
  const component2 = class Component extends HTMLElement {
    ...
    attributeChangedCallback(name3, _, value) {
      const decoded = config.attributes.get(name3)(value);
...
// build/dev/javascript/app/greet.mjs
function init(_) {
  let model = "";
  let effect = none();
  return [model, effect];
...
function view(model) {
  return div(toList([]), toList([text3("hello " + model)]));
}
function app() {
  return component(init, update2, view, toList([]));
}
var name = "greet-component";
